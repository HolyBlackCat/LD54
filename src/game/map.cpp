...
auto ShipGridData::GetRawTileInfoArray() -> std::array<TileInfo, std::to_underlying(Tile::_count)>
{
    ...
    std::array<TileInfo, std::to_underlying(Tile::_count)> ret = {{
        ...
        { .tile = Tile::piston_h, .solid = true,  .draw = SimpleTile{ivec2(0,1)}, .piston = PistonRelation::piston,           },
        { .tile = Tile::piston_v, .solid = true,  .draw = SimpleTile{ivec2(0,2)}, .piston = PistonRelation::piston,           },
    }};
